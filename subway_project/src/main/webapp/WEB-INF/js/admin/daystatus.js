...
$("#calendar").on("click",function(){
  $.ajax({
	    ...
	    success: function(data) {
			...
			lineChart(odayArray, opriceArray);
	    }
	});
...
// 검색하면 lineChart를 생성
function lineChart(odayArray, opriceArray){
  ...
  const config = {
    ...
    data: {
      ...
      datasets: [
        {
	        label: '일매출',  // 데이터셋의 이름
			data: opriceArray,  // 데이터셋의 데이터
			backgroundColor: 'rgba(153, 255, 153, 1)',  // 데이터셋의 배경색
			borderColor: 'rgba(0, 0, 0, 0.7)',  // 데이터셋의 테두리 색
			borderWidth: 3
        }
      ]
    },
